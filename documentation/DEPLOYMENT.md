...
| API | Railway ou Render | Gratuit |
| Base de données + Auth | Supabase | Gratuit |

**Ordre conseillé** : 1) Supabase → 2) API (Railway) → 3) Vercel (Frontend)
...
4. Dans **Redirect URLs**, ajoute :
   ...
   - `https://prompt-parfait-api-production.up.railway.app/**` (ou l’URL de ton API)
   - `http://localhost:5173/**` (pour le dev local)

---

## Étape 2 — Backend API (Railway ou Render)

### Option A — Railway

1. Va sur [railway.app](https://railway.app) → ton projet → service `prompt-parfait-api`
2. **Settings** → **Build** / **Service**
3. **Custom Build Command** : `cd server && npm install`
4. **Custom Start Command** : `cd server && npm start`  
   (Railway n’a pas de Root Directory visible, donc on passe par ces commandes)
5. **Variables** : `RIOT_API_KEY`, `FRONTEND_URL` = URL du frontend Vercel
...
4. **Environment Variables** → ajoute (déploie l’API Render avant pour avoir son URL) :
   - `VITE_DPM_API_URL` = `https://prompt-parfait-api-production.up.railway.app` (URL de ton API)
   - `VITE_SUPABASE_URL` = URL Supabase (Dashboard → Settings → API)
   - `VITE_SUPABASE_ANON_KEY` = clé anon Supabase
5. **Deploy**

> ⚠️ **« Supabase non configuré - Mode démo »** → Les variables `VITE_SUPABASE_URL` et `VITE_SUPABASE_ANON_KEY` sont absentes. Ajoute-les dans Vercel → projet → Settings → Environment Variables, puis **Redeploy**.

---

## Étape 4 — Vérifications

1. **HTTPS** : Vercel et Railway fournissent le SSL.
...
- **Railway** : plan gratuit (usage limité)

...
export function extractGameIdFromFilename(filename) {
  ...
  return numbers.length === 1 ? numbers[0] : numbers.reduce((a, b) => (b.length >= a.length ? b : a))
}
...
export function parseTimeline(raw, options = {}) {
  ...
  const gameIdFromFile = options.filename ? extractGameIdFromFilename(options.filename) : null
  ...
  const summary = {
    ...
    dragonsList: dragons.map((d) => ({
      ...
      killerId: d.killerId,
      label: MONSTER_LABELS[d.monsterSubType] || MONSTER_LABELS[d.monsterType] || d.monsterSubType || d.monsterType,
    })),

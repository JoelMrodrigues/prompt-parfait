 ...
 */
export function computeEnemyRoleFixes(enemyParticipants: any[]) {
  if (!enemyParticipants?.length) return []
  const list: any[] = enemyParticipants.map((p) => ({
    ...p,
  ...
  if (count.JUNGLE >= 2) {
    const jungles = list.filter(
      (p) => (p.role || '').toUpperCase().replace('JNG', 'JUNGLE') === 'JUNGLE'
    )
  ...
  const initial = new Map<string, string>(enemyParticipants.map((p) => [p.id, (String(p.role || '') || '').trim() || 'UNKNOWN']))
  return list
    .filter((p: any) => (initial.get(p.id) || '').toUpperCase() !== (String(p.role || '') || '').toUpperCase())

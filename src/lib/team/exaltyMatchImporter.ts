  ...
  for (const matchJson of matchesArray) {
    try {
      ...
      if (existing) {
        ...
        await supabase.from('team_match_participants').delete().eq('match_id', existing.id)
      ...
      for (const p of parsed.participants) {
        ...
        if (insertErr) {
          if (import.meta.env?.DEV) {
            console.error(
              '[import] team_match_participants insert error:',
              insertErr.code,
              insertErr.message,
              insertErr.details,
              row
          ...
          const colMissing =
            !useMinimalRow &&
            (String(insertErr.code) === '42703' ||
              /column.*does not exist|undefined_column/i.test(insertErr.message || ''))
          if (colMissing) {
            useMinimalRow = true
            ...
            if (retryErr) {
              results.errors.push(
                `${parsed.match.gameId} participant: ${retryErr.message}${retryErr.details ? ` (${retryErr.details})` : ''}`
              )
              insertFailed = true
              break
            }
          ...
          results.errors.push(
            `${parsed.match.gameId} participant: ${insertErr.message}${insertErr.details ? ` (${insertErr.details})` : ''}`
          )
          insertFailed = true
          break
        }

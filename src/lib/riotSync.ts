...
export async function fetchSyncRank(pseudo) {
  ...
  let res, data
  try {
    res = await apiFetch(`/api/riot/sync-rank?pseudo=${encodeURIComponent(pseudo.trim())}`)
    data = await res.json().catch(() => ({}))
  } catch (e) {
    const err = e as Error
    throw new Error(
      err.message?.includes('fetch') || err.message?.includes('Network')
        ? `API inaccessible. Vérifie VITE_DPM_API_URL et que l'API est déployée.`
...
export async function fetchWeeklyGames(pseudo: string): Promise<number | null> {
  ...
  try {
    const res = await apiFetch(`/api/riot/weekly-games?pseudo=${encodeURIComponent(pseudo.trim())}`)

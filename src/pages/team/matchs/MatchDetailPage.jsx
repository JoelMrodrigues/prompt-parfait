            ...
            <tr key={p.id} className="border-b border-dark-border/50 hover:bg-dark-bg/30">
              <td className="py-3 px-4">
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <div className="font-medium">{p.champion_name}</div>
                    {p.pseudo && (
                      <div className="text-xs text-gray-500">{p.pseudo}</div>
                    )}
                  </div>
                </div>
              </td>
...
export const MatchDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchMatch = async () => {
      try {
        loadItems()
          .then(() => { if (!cancelled) setItemsLoaded(true) })
          .catch(() => { if (!cancelled) setItemsLoaded(true) })
        itemsTimeout = setTimeout(() => { if (!cancelled) setItemsLoaded(true) }, 3000)
        ...
        const [{ data: parts, error: partsError }, { data: timelineRow }] = await Promise.all([
          supabase.from('team_match_participants').select('*').eq('match_id', matchId),
          supabase.from('team_match_timeline').select('snapshot').eq('match_id', matchId).maybeSingle(),
        ])
      ...
      <div className="bg-dark-card border border-dark-border rounded-xl overflow-hidden mb-6">
        <div className="p-6 border-b border-dark-border">
          <div className="flex flex-wrap items-center gap-4">
            ...
            {match.game_mode && (
              <span className="text-gray-500 text-sm">{match.game_mode}</span>
            )}
            {match.game_creation && (
        ...
        {activeTab === 'global' && (
          <div className="p-6 space-y-8">
            {/* 1) Stats par joueur */}
            <div>
              <h3 className="font-display text-lg font-semibold text-white mb-2">Stats globales — par joueur</h3>
                          ...
                          <tr key={p.id} className="border-b border-dark-border/50 hover:bg-dark-bg/30">
                            <td className="py-2 px-3">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <div className="font-medium">{p.champion_name}</div>
                                  {p.pseudo && <div className="text-[11px] text-gray-500">{p.pseudo}</div>}
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="py-2 px-3 text-right">{kp != null ? `${kp.toFixed(0)}%` : '—'}</td>
                            <td className="py-2 px-3 text-right">
                              {dmg ? dmg.toLocaleString() : '—'}
                            </td>
                            <td className="py-2 px-3 text-right">{dmgShare != null ? `${dmgShare.toFixed(0)}%` : '—'}</td>
                            <td className="py-2 px-3 text-right">
            ...
            <div>
              <h3 className="font-display text-lg font-semibold text-white mb-2">Stats globales — par équipe</h3>
              <p className="text-gray-500 text-sm mb-4">
              ...
              <div className="mt-4 p-3 bg-dark-bg/50 rounded-lg text-gray-400 text-sm">
                <strong>Avantage final :</strong>{' '}
                Or {ourGold >= enemyGold ? '+' : ''}
          ...
          <div className="p-6 space-y-8">
            ...
            {timelineSnapshot && Object.keys(timelineSnapshot).length > 0 ? (
              <>
                ...
                <div>
                  <h4 className="font-display font-semibold text-green-400/90 mb-2">Avantages par joueur</h4>
                  <p className="text-gray-500 text-sm mb-3">Avantage vs son vis-à-vis (1 vs 6, 2 vs 7, …). + = vert, − = rouge. CS = sbires + monstres (jungle incluse).</p>
                  {timelineSnapshot[TIMELINE_MINUTES[0]]?.participants ? (
                    <div className="space-y-6">
                      <div className="overflow-x-auto">
                        <p className="text-gray-400 text-xs mb-1">Notre équipe (participants 1-5)</p>
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="text-left text-gray-400 border-b border-dark-border">
                              <th rowSpan={2} className="py-2 px-2 sticky left-0 bg-dark-card align-bottom">Joueur</th>
                              {TIMELINE_MINUTES.map((m) => (
                                <th key={m} colSpan={3} className="py-1 px-1 text-center text-xs border-l border-dark-border/70 bg-dark-bg/50">
                            ...
                            <tr className="text-gray-400 border-b border-dark-border">
                              {TIMELINE_MINUTES.flatMap((m) => [
                                <th key={`${m}-or`} className="py-1 px-0.5 text-right text-xs border-l border-dark-border/50 w-14">Or</th>,
                                <th key={`${m}-xp`} className="py-1 px-0.5 text-right text-xs w-14">XP</th>,
                                <th key={`${m}-cs`} className="py-1 px-0.5 text-right text-xs w-12">CS</th>,
                              ])}
                            </tr>
                          </thead>
                          <tbody>
                            {ourTeam.map((part, i) => {
                              ...
                              return (
                                <tr key={part?.id ?? ourPid} className="border-b border-dark-border/50">
                                  <td className="py-2 px-2 sticky left-0 bg-dark-card">
                                    <div className="flex items-center gap-2">
                                      <img
                                        src={getChampionImage(part?.champion_name)}
                                        alt={part?.champion_name || ''}
                                        className="w-8 h-8 rounded object-cover"
                                      />
                                      <div>
                                        <div className="font-medium">{part?.champion_name || `Participant ${ourPid}`}</div>
                                        {part?.pseudo && <div className="text-xs text-gray-500">{part.pseudo}</div>}
                                      </div>
                                    </div>
                                  </td>
                                  {TIMELINE_MINUTES.map((m) => {
                                    ...
                                    return (
                                      <Fragment key={m}>
                                        <td className={`py-2 px-0.5 text-right border-l border-dark-border/50 ${goldDiff > 0 ? 'text-green-400' : goldDiff < 0 ? 'text-red-400' : 'text-gray-400'}`}>
                                          {goldDiff >= 0 ? '+' : ''}{goldDiff.toLocaleString()}
                                        </td>
                                        <td className={`py-2 px-0.5 text-right ${xpDiff > 0 ? 'text-green-400' : xpDiff < 0 ? 'text-red-400' : 'text-gray-400'}`}>
                                          {xpDiff >= 0 ? '+' : ''}{xpDiff.toLocaleString()}
                                        </td>
                                        <td className={`py-2 px-0.5 text-right ${csDiff > 0 ? 'text-green-400' : csDiff < 0 ? 'text-red-400' : 'text-gray-400'}`}>
                                          {csDiff >= 0 ? '+' : ''}{csDiff}
                                        </td>
                                      </Fragment>
                                    )
                      ...
                      <div className="overflow-x-auto">
                        <p className="text-gray-400 text-xs mb-1">Équipe adverse (participants 6-10)</p>
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="text-left text-gray-400 border-b border-dark-border">
                              <th rowSpan={2} className="py-2 px-2 sticky left-0 bg-dark-card align-bottom">Joueur</th>
                          ...
                          <tbody>
                            {enemyTeam.map((part, i) => {
                              ...
                              const getCs = (s) => ((s?.minions ?? 0) + (s?.jungle ?? 0)) || (s?.cs ?? 0)
                              return (
                                <tr key={part?.id ?? enemyPid} className="border-b border-dark-border/50">
                                  <td className="py-2 px-2 sticky left-0 bg-dark-card">
                                    <div className="flex items-center gap-2">
                                      <img
                                        src={getChampionImage(part?.champion_name)}
                                      ...
                                      />
                                      <div>
                                        <div className="font-medium">{part?.champion_name || `Participant ${enemyPid}`}</div>
                                        {part?.pseudo && <div className="text-xs text-gray-500">{part.pseudo}</div>}
                                      </div>
                                    </div>
                                  </td>
                    ...
                    <p className="text-gray-500 text-sm">Réimportez la timeline pour ce match pour afficher les stats par joueur (format mis à jour).</p>
                  )}
                </div>

                {/* Partie 2 : Avantage global équipe (même principe que Stats globales) */}
                <div>
                  <h4 className="font-display font-semibold text-white mb-2">Avantage global équipe</h4>
                  <p className="text-gray-500 text-sm mb-3">Totaux or / XP par équipe à 5, 10, 15, 20, 25 minutes.</p>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-left text-gray-400 border-b border-dark-border">
                        ...
                        {TIMELINE_MINUTES.filter((m) => timelineSnapshot[String(m)]).map((min) => {
                          ...
                          return (
                            <tr key={min} className="border-b border-dark-border/50">
                              ...
                              <td className="py-2 px-3 text-right">{(s.gold_100 ?? 0).toLocaleString()}</td>
                              <td className="py-2 px-3 text-right">{(s.gold_200 ?? 0).toLocaleString()}</td>
                              <td className={`py-2 px-3 text-right ${goldDiff >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                {goldDiff >= 0 ? '+' : ''}{goldDiff.toLocaleString()}
                              </td>
                              <td className="py-2 px-3 text-right">{(s.xp_100 ?? 0).toLocaleString()}</td>
                              <td className="py-2 px-3 text-right">{(s.xp_200 ?? 0).toLocaleString()}</td>
                              <td className={`py-2 px-3 text-right ${xpDiff >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                {xpDiff >= 0 ? '+' : ''}{xpDiff.toLocaleString()}
                              </td>
                            </tr>
                          )
              ...
              <div className="p-6 bg-dark-bg/50 rounded-lg border border-dark-border text-center text-gray-400">
                ...
                <p className="text-sm">Importez un fichier timeline (ex. timeline3.txt) et associez-le à ce match depuis la page Import pour afficher l’avantage or/XP à 5, 10, 15, 20, 25 min.</p>
              </div>
            )}
          </div>
        )}

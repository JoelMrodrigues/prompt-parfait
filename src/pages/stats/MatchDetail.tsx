...
export const MatchDetail = () => {
  ...
  return (
    <div className="w-full px-6 py-12">
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`px-6 py-3 font-semibold transition-all relative ${
              activeTab === tab.id ? 'text-accent-blue' : 'text-gray-400 hover:text-gray-300'
            }`}
          >
            {tab.label}
            {activeTab === tab.id && (
      ...
      >
        {activeTab === 'summary' && <SummaryTab matchData={matchData} />}
        {activeTab === 'allstats' && <AllStatsTab matchData={matchData} />}
        {activeTab === 'builds' && <BuildsTab matchData={matchData} />}
        {activeTab === 'timeline' && <TimelineTab matchData={matchData} />}
        {activeTab === 'plays' && <PlaysTab matchData={matchData} />}
      </motion.div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-6">
        {/* Blue Team */}
        <div
          className={`bg-dark-card border rounded-lg p-6 ${isBlueWinner ? 'border-green-500' : 'border-red-500'}`}
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold">{blueTeam.name}</h3>
            <span
              className={`px-3 py-1 rounded text-sm font-semibold ${isBlueWinner ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'}`}
            >
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              {blueTeam.players[0] &&
                ['ban1', 'ban2', 'ban3', 'ban4', 'ban5'].map((ban, idx) => (
                  <div
                    key={idx}
                    className="w-12 h-12 bg-dark-bg rounded border border-dark-border flex items-center justify-center"
                  >
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center">
                    ...
                    >
                      {(player.csdiffat15 || 0) > 0 ? '+' : ''}
                      {(player.csdiffat15 || 0).toFixed(0)}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span
                      className={
                        (player.golddiffat15 || 0) > 0
                          ? 'text-green-500'
                          : (player.golddiffat15 || 0) < 0
                            ? 'text-red-500'
                            : ''
                    ...
                    >
                      {(player.golddiffat15 || 0) > 0 ? '+' : ''}
                      {(player.golddiffat15 || 0).toFixed(0)}
                    </span>
                  </td>
                </tr>
              )

  ...
  return (
    <div className="bg-dark-card border border-dark-border rounded-lg overflow-hidden w-full">
        ...
        <table className="w-full min-w-full">
          <thead>
            <tr className="bg-[#4A6B7C]">
              {COLUMNS.map((column) => (
                ...
                >
                  <div
                    className={`flex items-center gap-1 ${column.align === 'left' ? 'justify-start' : 'justify-center'}`}
                  >
          ...
          <tbody>
            {sortedData.map((row, index) => (
              <tr
                key={row.champion}
                className={`border-b border-dark-border hover:bg-dark-bg/50 transition-colors ${index % 2 === 0 ? 'bg-[#2C3E50]' : 'bg-[#34495E]'}`}
              >
                <td className="px-4 py-4 text-left align-middle">
                  ...
                  >
                    ...
                    />
                    <span className="font-medium text-white hover:text-accent-blue transition-colors">
                      {row.champion}
                    </span>
                  </Link>
                </td>
                ...
                <td className="px-4 py-4 text-center align-middle text-white">
                  <span
                    className={
                      row.csd15 > 0 ? 'text-green-400' : row.csd15 < 0 ? 'text-red-400' : ''
                    }
                  >
                    {row.csd15.toFixed(1)}
                  </span>
                ...
                <td className="px-4 py-4 text-center align-middle text-white">
                  <span
                    className={row.gd15 > 0 ? 'text-green-400' : row.gd15 < 0 ? 'text-red-400' : ''}
                  >
                    {row.gd15}
                  </span>
                </td>
      ...
      <div className="bg-[#4A6B7C] px-4 py-3 text-sm text-white text-center">
        {sortedData.length} champion{sortedData.length > 1 ? 's' : ''} affiché
